...
%[nlZ, dnlZ, a, b, c, post] = gp(hyp, @infGaussLik, meanfunc, covfunc, likfunc, x, y, xs);

%postK = inv(inv(K) + post.sW);
...
figure(4);
axes('Box','off', 'Units','inches','Position',[1.5 1.5 4 4]);
hold on;
imshow(K, 'XData', x, 'YData', x);
colormap(hot);
cb = colorbar;
cb.Label.String = 'k(x, x'')';
xlim([-5, 5]);
ylim([-5, 5]);
xlabel('x');
ylabel('x''');
