...
def LDA(A, B, K, alpha, gamma, num_iter):
    ...
    # Initialization
    skd = np.zeros((K, D))  # count of word assignments to topics for document d (theta)
    swk = np.zeros((W, K))  # unique word topic assignment counts across all documents (not quite beta), K topics, W words
    ...
    topic_posts = np.zeros((D, K, num_iter))
    word_entropy = np.zeros((K, num_iter))
    beta = np.zeros((W, K))
    
    num_gibbs_iters = num_iter
    for iter in range(num_gibbs_iters):
        ...
        for d in range(D):
            ...
            topic_posts[d,:,iter] = (skd[:,d].flatten() + alpha) / sum(skd[:,d].flatten() + alpha)
        for k in range(K):   
            beta[:, k] = (swk[:, k] + gamma) / sum(swk[:, k] + gamma)
        word_entropy[:, iter] = np.diag(np.matmul(beta.T, np.log(beta)))
    ...
    lp = np.zeros((len(unique_docs_in_b), num_iter))
    nd = []
    for index, d in enumerate(unique_docs_in_b):  # loop over all documents in B
        ...
        for iters in range(num_gibbs_iters):
            ...
            lp[index, iters] = np.dot(np.log(b[words_and_counts[:, 0]-1]).T, words_and_counts[:, 1])  # log probability, doc d
                      
        nd.append(np.sum(words_and_counts[:, 1]))  # number of words, doc d

    perplexity = np.exp(-np.sum(lp, axis=0)/sum(nd))  # perplexity

    return perplexity, swk, topic_posts, word_entropy
