...
def BMM(A, B, K, alpha, gamma, num_iter, seed):
    ...
    sk_docs = np.zeros((K, 1), dtype=int)  # number of documents assigned to each mixture i.e. mixing counts
    ...
    num_iters_gibbs = num_iter
    
    mix_probs = np.zeros((K, num_iters_gibbs))
    ...
    for iter in range(num_iters_gibbs):
        ...
        mix_probs[:, iter] = (sk_docs.flatten() + alpha) / sum(sk_docs.flatten() + alpha)
    # test documents
    lp = 0
    nd = 0
    ...
    return perplexity, swk, mix_probs
